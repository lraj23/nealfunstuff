...
const funcs = [
	...
	},
	{
		itemName: "📅 Next month",
		updateRate: msInDay,
		totalTime: _ => new Date(...(d => [d.getFullYear(), d.getMonth() + 1, 0])(new Date())).getDate() * msInDay,
	...
	},
	{
		itemName: "🎊 Next year",
		updateRate: msInDay,
		totalTime: _ => (year => new Date(year, 1, 29).getDate() === 29 ? 366 : 365)(new Date().getFullYear()) * msInDay,
	...
	},
	{
		...
		updateRate: msInYr(0),
		totalTime: _ => (year => new Array(10).fill((Math.floor(year / 10) * 10) - year).map((yr, i) => msInYr(yr + i)).reduce((a, b) => a + b))(new Date().getFullYear()),
		determineTime: time => new Date(Math.ceil(new Date().getFullYear() / 10) * 10, 0) - time + offset()
	},
	{
		itemName: "💯 Next century",
		updateRate: msInYr(0),
		totalTime: _ => (year => new Array(100).fill((Math.floor(year / 100) * 100) - year).map((yr, i) => msInYr(yr + i)).reduce((a, b) => a + b))(new Date().getFullYear()),
		determineTime: time => new Date(Math.ceil(new Date().getFullYear() / 100) * 100, 0) - time + offset()
	},
	{
		itemName: "🔟0️⃣0️⃣ Next millenium",
		updateRate: msInYr(0),
		totalTime: _ => (year => new Array(1000).fill((Math.floor(year / 1000) * 1000) - year).map((yr, i) => msInYr(yr + i)).reduce((a, b) => a + b))(new Date().getFullYear()),
